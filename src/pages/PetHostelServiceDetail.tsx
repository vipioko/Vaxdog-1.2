...
const PetHostelServiceDetail = () => {
  const { serviceId } = useParams<{ serviceId: string }>();
  const navigate = useNavigate();
  const { services, isLoading } = usePetHostelServices();
        ...
        <div>
          ...
          <div className="flex items-center justify-between">
            <div className="text-2xl sm:text-3xl font-bold text-blue-400">
              <IndianRupee className="inline-block h-6 w-6 mr-1" />{service.dailyRate.toFixed(2)}/day
            </div>
        ...
        <Card className="bg-slate-800/50 border-slate-700">
          <CardContent className="p-4">
            ...
            <div className="space-y-2 text-sm text-slate-300">
              <p>• Secure and comfortable boarding environment</p>
              <p>• Experienced and caring staff</p>
              <p>• Daily updates and photos of your pet</p>
              <p>• Emergency veterinary support available</p>
            </div>
          </CardContent>
        </Card>
      </div>
