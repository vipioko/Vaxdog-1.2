...
const GroomingBookingManagement = () => {
  const { adminGroomingBookings, isLoading, updateGroomingBookingStatus, isUpdatingGroomingBookingStatus } = useGroomingBookings();
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedBooking, setSelectedBooking] = useState<GroomingBooking | null>(null);
  const [statusUpdateBooking, setStatusUpdateBooking] = useState<GroomingBooking | null>(null);
  ...
  const filteredBookings = adminGroomingBookings.filter(booking =>
  ...
  const handleViewDetails = (booking: GroomingBooking) => {
    setSelectedBooking(booking);
    setIsDetailsDialogOpen(true);
  };

  const handleUpdateStatus = (booking: GroomingBooking) => {
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      await updateGroomingBookingStatus({
        userId: statusUpdateBooking.userId,
        bookingId: statusUpdateBooking.id,
        status: newStatus as GroomingBooking['bookingStatus'],
      });
  ...
  if (isLoading) {
    return (
      <Card className="bg-slate-800/50 border-slate-700">
        <CardHeader>
          <CardTitle className="text-white">Grooming Bookings</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <CardHeader>
        <CardTitle className="flex items-center justify-between text-white">
          ...
          <Badge variant="secondary" className="bg-purple-500/20 text-purple-300">
            {adminGroomingBookings.length} Bookings
          </Badge>
        </CardTitle>
      </CardHeader>
      <CardContent>
        ...
        {filteredBookings.length === 0 ? (
          <div className="text-center py-16">
            ...
            <h3 className="text-lg font-medium text-white mb-2">
              {adminGroomingBookings.length === 0 ? 'No grooming bookings yet' : 'No bookings found'}
            </h3>
            <p className="text-slate-400 text-sm">
              {adminGroomingBookings.length === 0
                ? 'Grooming bookings will appear here when customers place them'
            ...
            <TableBody>
              {filteredBookings.map((booking) => (
                <TableRow key={booking.id} className="border-slate-700">
                  ...
                  <TableCell className="text-white">
                    <p>{booking.preferredDate ? format(new Date(booking.preferredDate), 'MMM d, yyyy') : 'N/A'}</p>
                    <p className="text-sm text-slate-400">{booking.preferredTime || 'N/A'}</p>
                  </TableCell>
                  <TableCell>
        ...
        <Dialog open={isDetailsDialogOpen} onOpenChange={setIsDetailsDialogOpen}>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto bg-slate-800 border-slate-700 text-white">
            <DialogHeader>
              <DialogTitle className="text-white">Grooming Booking Details - {selectedBooking?.serviceName}</DialogTitle>
            </DialogHeader>
            {selectedBooking && (
